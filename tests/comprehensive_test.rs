...
async fn test_comprehensive_mysql_compatibility() {
    ...
    for (i, statement) in statements.iter().enumerate() {
        ...
        let test_name = match statement.to_lowercase().as_str() {
            s if s.contains("create table test_users") => "Test 1: Basic table creation with AUTO_INCREMENT",
            ...
            s if s.contains("left join") && s.contains("count") => "Test 10: LEFT JOIN with aggregation",
    ...
    assert!(passed_tests > failed_tests, "More tests should pass than fail");
    assert!(passed_tests >= (passed_tests + failed_tests) / 2, "At least 50% of tests should pass");
}
