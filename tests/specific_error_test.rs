...
async fn test_specific_error_case() {
    ...
    // Clean up
    let _ = query_handler.handle_query("DROP TABLE IF EXISTS test").await;
    let _ = query_handler.handle_query("DROP TABLE IF EXISTS test_corrected").await;
...
async fn test_regex_fixing() {
    ...
    let test_cases = vec![
        ("CREATE TABLE test(name(VARCHAR255))", "CREATE TABLE test(name VARCHAR(255))"),
    ...
    for (input, expected) in test_cases {
        ...
        println!("Input: {} -> Fixed: {} (Expected: {})", input, fixed, expected);
