...
async fn test_server_startup_and_connection() {
    ...
    match connection_result {
        Ok(Ok(_stream)) => {
            println!("✓ Successfully connected to PostMyRustache server on {}", config.bind_address);
...
async fn test_multiple_connections() {
    ...
    println!("✓ Successfully handled {}/3 concurrent connections", successful_connections);
