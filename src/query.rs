...
impl QueryHandler {
    ...
    fn handle_mysql_specific_query(&self, sql: &str) -> Option<QueryResult> {
        ...
        if sql_trimmed.contains("@@session.") || sql_trimmed.contains("@@global.") {
            log::info!(
                "Intercepted MySQL session/global variable query, returning dummy response."
            );
            return Some(QueryResult::Ok(OkResponse::default()));
        }
        ...
        if sql_trimmed.contains("now()")
            || sql_trimmed.contains("curdate()")
            || sql_trimmed.contains("curtime()")
        {
    ...
    fn translate_mysql_to_postgres(&self, sql: &str) -> String {
        ...
        if translated.to_lowercase().contains("create database")
            && translated.to_lowercase().contains("if not exists")
        {
    ...
    fn fix_common_sql_errors(&self, sql: &str) -> String {
        ...
        if fixed.contains("CREATE TABLE") {
            ...
            if let Ok(re) =
                Regex::new(r"\)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+(VARCHAR|CHAR|INT|TEXT|TIMESTAMP)")
            {
                fixed = re.replace_all(&fixed, "), $1 $2").to_string();
            }
        }
